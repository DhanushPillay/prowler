...
class monitor_activity_log_alert_create_update_delete_network_sg(Check):
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription, alert_rules in monitor_client.alert_rules.items():
            ...
            report.status_extended = f"There is not an active log alert for 'monitor activity log alert create update delete network sg' in subscription {subscription}."
            ...
            for alert_rule in alert_rules:
                ...
                for condition in alert_rule.condition.all_of:
                    ...
                    if condition.field == "operationName":
                        ...
                        if op_name in ['microsoft.network/networksecuritygroups/write']:
                            has_write = True
                        if op_name in ['microsoft.network/networksecuritygroups/delete']:
                            has_delete = True
            ...
            if write_covered and delete_covered:
                report.status = "PASS"
                report.status_extended = f"There is an active log alert for 'monitor activity log alert create update delete network sg' in subscription {subscription}."
