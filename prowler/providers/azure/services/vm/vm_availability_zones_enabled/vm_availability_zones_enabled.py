...
class vm_availability_zones_enabled(Check):
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, virtual_machines in vm_client.virtual_machines.items():
            for vm_id, vm in virtual_machines.items():
                ...
                if vm.zones:
                    report.status = "PASS"
                    report.status_extended = f"VM {vm.resource_name} has availability zones enabled."
                else:
                    report.status = "FAIL"
                    report.status_extended = f"VM {vm.resource_name} does not have availability zones enabled."
