...
class recovery_services_vault_uses_private_link(Check):
    def execute(self) -> Check_Report_Azure:
        ...
        for subscription_id, vaults in recovery_client.vaults.items():
            for vault_id, vault in vaults.items():
                ...
                if has_approved_pec:
                    report.status = "PASS"
                    report.status_extended = f"Recovery Services Vault {vault.name} uses an approved private link."
                else:
                    report.status = "FAIL"
                    report.status_extended = f"Recovery Services Vault {vault.name} does not use an approved private link."
