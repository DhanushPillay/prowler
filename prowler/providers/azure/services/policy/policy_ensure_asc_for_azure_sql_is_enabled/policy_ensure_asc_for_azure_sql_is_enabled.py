...
class policy_ensure_asc_for_azure_sql_is_enabled(Check):
    """Check if Azure Security Center is enabled for Azure SQL via Policy.

    This check verifies if the Azure Security Center Default Policy (or SecurityCenterBuiltIn) has the pricingTierSqlDb parameter set to Standard, ensuring that Defender for Azure SQL is enabled.
    """

    def execute(self) -> list[Check_Report_Azure]:
        """Execute policy_ensure_asc_for_azure_sql_is_enabled check.
        ...
        for subscription_id, assignments in policy_client.policy_assigments.items():
            ...
            if asc_policy:
                ...
                param_val = None
                if asc_policy.parameters and "pricingTierSqlDb" in asc_policy.parameters:
                    raw_val = asc_policy.parameters["pricingTierSqlDb"]
                ...
                if param_val == "Standard" and asc_policy.enforcement_mode == "Default":
                    ...
                    report.status_extended = (
                        f"ASC Default policy '{asc_policy.name}' has 'pricingTierSqlDb' set to Standard with enforcement enabled."
                    )
                elif param_val == "Standard":
                    report.status = "FAIL"
                    report.status_extended = (
                        f"ASC Default policy '{asc_policy.name}' has 'pricingTierSqlDb' set to Standard but enforcement is disabled."
                    )
                else:
                    report.status = "FAIL"
                    report.status_extended = (
                        f"ASC Default policy '{asc_policy.name}' does not have 'pricingTierSqlDb' set to Standard."
                    )
            else:
                report.status = "FAIL"
                report.status_extended = (
